...
export default function Battleship({ isDragging, color, position,onDrop, stateCouleur }) {
    ...
    const [boatSquares, setSquares] = useState(Array(4).fill(null));
    ...
    function fabriqueBattleship(position) {
        return (<BoatSquare value={boatSquares[position]} click={() => onClickBoat(position)}/>);
    }

    return(
        <div className="boat">
            <div className="battleship" ref={dragRef} style={{ opacity, backgroundColor:color}}>
                <div className={isActive ? "boat-row" : "boat-col"} >
                    {fabriqueBattleship(0)}
                    {fabriqueBattleship(1)}
                    {fabriqueBattleship(2)}
                    {fabriqueBattleship(3)}
                </div>
            </div>
